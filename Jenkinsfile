...
def SERVICE_NAME = "geolocation-service"
...
volumes: [
  ...
  node(label) {
    ...
    stage('Clone git') {
          git credentialsId: 'jenkins-github', url: "https://github.com/surfadvisor/geolocation-service"
    }

    stage('Download .jar from S3') {
          withAWS(region: 'eu-central-1', credentials: 'aws-code-ro-access') {
           s3Download(file:'app.jar', bucket:"${CODE_REPO}", path:"release/com/surf/advisor/geolocation/application/${version}/application-${version}.jar", force:true)
          }
          sh 'chmod +x app.jar'
          sh 'ls -al'
    }
